...
function initAct() {
    ...
    var first = false; while (!first) {
        if ($("#C" + col.toString() + "_" + row.toString()).length) {
            first = true;
            $("#C" + col.toString() + "_" + row.toString()).focus(); $("#C" + col.toString() + "_" + row.toString()).css("backgroundColor", colorSele);
        } else {
    ...
    $(".cell").click(function (e) {
        var idCell = $(this).attr("id").substring(1); var pos = idCell.indexOf("_");
    ...
    $(".cell").keydown(function (e) {
        er = "|^[a-zA-ZñÑáéíóúÁÉÍÓÚüÜ]*$|"; if (e.keyCode != 37 && e.keyCode != 38 && e.keyCode != 39 && e.keyCode != 40 && e.keyCode != 9) { $(this).val(String.fromCharCode(e.keyCode).match(er)); }
...
function writeDef() {
    var newDef = ""; var newIma = ""; var newAudio = -1; $("#defIma").css("visibility", "hidden");
    for (i = 0; i < x1.length; i++) {
        if (y1[i] == y2[i] && y1[i] == row + 1 && x1[i] <= col + 1 && x2[i] >= col + 1 && writeDir == 0) {
            newDef = "<div class='titleDef'>Horizontales:<br></div>" + defCW[i]; if (imaCW[i].localeCompare("") != 0) {
                newIma = '<img alt="' + altCW[i] + '" class="imaLeft" src="' + imaCW[i] + '" align="left"><div style="clear:both;"><div>';
            } if (audioCW[i].localeCompare("") != 0) { newAudio = i; }
        }
        if (x1[i] == x2[i] && x1[i] == col + 1 && y1[i] <= row + 1 && y2[i] >= row + 1 && writeDir == 1) {
            newDef = "<div class='titleDef'>Verticales:<br></div>" + defCW[i];
            if (imaCW[i].localeCompare("") != 0) {
                newIma = '<img alt="' + altCW[i] + '" class="imaLeft" src="' + imaCW[i] + '" align="left"><div style="clear:both;"><div>';
            } if (audioCW[i].localeCompare("") != 0) { newAudio = i; }
        }
    } if (newDef.localeCompare("") == 0 && newIma.localeCompare("") == 0) { $("#defIma").css("visibility", "hidden"); }
...
function paintButDir() {
    var canvas = document.getElementById("ardoraButtonDirCanvas"); var contexto = canvas.getContext("2d"); contexto.fillStyle = colorButton;
...
function paintButDirIn() {
    var canvas = document.getElementById("ardoraButtonDirCanvas"); var contexto = canvas.getContext("2d");
...
function goCell(toDir) {
    var newCol = col; var newRow = row; if (toDir == 37) { newCol--; } if (toDir == 38) { newRow--; }
    if (toDir == 39) { newCol++; } if (toDir == 40) { newRow++; } if ($("#C" + newCol.toString() + "_" + newRow.toString()).length) {
        col = newCol; row = newRow; $("#C" + col.toString() + "_" + row.toString()).focus();
    } writeDef();
}
function nextCell() {
    if (writeDir == 0) { if ($("#C" + (col + 1).toString() + "_" + row.toString()).length) { col++; $("#C" + col.toString() + "_" + row.toString()).focus(); } }
    else { if ($("#C" + col.toString() + "_" + (row + 1).toString()).length) { row++; $("#C" + col.toString() + "_" + row.toString()).focus(); } }
}
function randomSort() {
}
function isCorrect() {
    var correct = true; for (i = 0; i < rowNum; i++) {
        for (j = 0; j < colNum; j++) {
            if (cw[i][j].localeCompare("") != 0 && CWWords(cw[i][j]).substring(0, 1).localeCompare(":") != 0) {
...
function showSol(oldTypeGame) {
    for (i = 0; i < rowNum; i++) {
        for (j = 0; j < colNum; j++) {
            if (cw[i][j].localeCompare("") != 0 && CWWords(cw[i][j]).localeCompare(":") != 0) {
