...
var api = null; var apiHandle = null; var apiAttempts = 0; var apiStatus = ""; var apiInput = 0; function loadPage() {
    ...
    if (api == null) { } else {
        api.LMSInitialize(""); apiStatus = api.LMSGetValue("cmi.core.lesson_status"); if ((apiStatus != "failed") && (apiStatus != "passed")) {
            api.LMSSetValue("cmi.core.lesson_status", "failed");
        } else {
            if ((scoOk == 0) && (scoWrong == 0)) { api.LMSSetValue("cmi.core.lesson_status", "failed"); }
            else {
                if (apiStatus == "passed") {
                    if (scoOk == 0) { api.LMSSetValue("cmi.core.lesson_status", "failed"); } else {
                        if (tiButtonTime) {
                ...
                if (apiStatus == "failed") {
                    if (scoWrong == 0) { api.LMSSetValue("cmi.core.lesson_status", "failed"); } else {
                        if (tiButtonTime) {
                            $("#ardoraActCanvas").unbind("mousedown"); var canvas = document.getElementById("ardoraActCanvas"); canvas.width = canvas.width;
                        }
        ...
        $("#ardoraAlumSCORM").click(function () {
            var widthAlum = $("#ardoraAlumSCORM").css("width");
...
function unloadPage() {
    if ((api != null) && (apiInput == 0)) {
        var currentTime = new Date(); var endTime = currentTime.getTime()
        ...
        if (totalHours < 1000 && totalHours > 99) { totalHours = "0" + totalHours; } else if (totalHours < 100 && totalHours > 9) {
        ...
        if (successes == successesMax) {
            api.LMSSetValue("cmi.core.lesson_status", "passed"); if (scoOkDo == 1) { api.LMSSetValue("cmi.core.score.raw", scoPtos); }
            else { if (score > 0) { api.LMSSetValue("cmi.core.score.raw", score) } else { api.LMSSetValue("cmi.core.score.raw", 0) } }
        } else {
            if (scoWrongDo == 1) {
